    ...
    <div id="tabelPrint" hidden style="font-size:13px;width:100%">
        <table id="" class="table table-bordered table-hover mx-auto"  >
          ...
          <tbody>
            ...
            <?php 
            $saldo = "SELECT SUM(t.harga) as total FROM (SELECT harga,package FROM data_history_cobahampirfinishjuga WHERE tanggal LIKE '%2022-08%' AND studio = 'Bekasi-Kota' UNION ALL SELECT harga,package FROM data_history WHERE tanggal LIKE '%2022-08%' AND studio = 'Bekasi-Kota') t";
            $total = mysqli_query($koneksi,$saldo);
            while($row = mysqli_fetch_assoc($total)){
            $output = $row['total'];
            }
            $data = mysqli_query($koneksi, "SELECT * FROM data_history_cobahampirfinishjuga WHERE tanggal LIKE '%2022-08%' AND studio = 'Bekasi-Kota' UNION SELECT * FROM data_history WHERE tanggal LIKE '%2022-08%' AND studio = 'Bekasi-Kota' ORDER BY id DESC");
            $nomor = 1;
          ...
          <?php  
                ...
                // $amal = $d["harga"]*(0.025);
                //////////////////////////////
                    $saldo1 = "SELECT SUM(t.harga) as total FROM (SELECT harga,tipe_package FROM data_history_cobahampirfinishjuga WHERE tanggal LIKE '%2022-08%' AND studio = 'Bekasi-Kota' UNION ALL SELECT harga,tipe_package FROM data_history WHERE tanggal LIKE '%2022-08%' AND studio = 'Bekasi-Kota') t WHERE tipe_package = 'prawedding'";
                    ...
                    $saldo2 = "SELECT SUM(t.harga) as total FROM (SELECT harga,package FROM data_history_cobahampirfinishjuga WHERE tanggal LIKE '%2022-08%' AND studio = 'Bekasi-Kota' UNION ALL SELECT harga,package FROM data_history WHERE tanggal LIKE '%2022-08%' AND studio = 'Bekasi-Kota') t WHERE package IN ('self photo','studio')";
                    ...
                    $saldo3 = "SELECT SUM(t.harga) as total FROM (SELECT harga,package,tipe_package FROM data_history_cobahampirfinishjuga WHERE tanggal LIKE '%2022-08%' AND studio = 'Bekasi-Kota' UNION ALL SELECT harga,package,tipe_package FROM data_history WHERE tanggal LIKE '%2022-08%' AND studio = 'Bekasi-Kota') t WHERE tipe_package IN ('best deal','special package','diamond package','baby best deal','baby special package')";
                    ...
                    $saldo4 = "SELECT SUM(t.harga) as total FROM (SELECT * FROM data_history_cobahampirfinishjuga WHERE tanggal LIKE '%2022-08%' AND studio = 'Bekasi-Kota' UNION ALL SELECT * FROM data_history WHERE tanggal LIKE '%2022-08%' AND studio = 'Bekasi-Kota') t WHERE package = 'cetak foto'";
