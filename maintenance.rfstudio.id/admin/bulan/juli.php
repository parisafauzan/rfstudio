...
<div class="row mb-3 mt-3">
  ...
  <div class="col ">
    <form method="post" action="juli-excel.php">
...
<div class="container-md align-middle">
	<div class="table-responsive " id="printableArea">
        ...
        <h5 class="text-center">- Seluruh data : Bulan Juli -</h5>
		...
		<div class="table-wrapper" id="table1">
          ...
          $saldo = "SELECT SUM(harga) AS sum FROM data_history WHERE YEAR(tanggal) = YEAR(NOW()) AND MONTH(tanggal) IN (7)";
          ...
          $output = $row['sum'];
            ...
            $data = mysqli_query($koneksi, "SELECT * FROM data_history WHERE YEAR(tanggal) = YEAR(NOW()) AND MONTH(tanggal) IN (7) ORDER BY tanggal DESC");
            $jumlah_data = mysqli_num_rows($data);
            $total_halaman = ceil($jumlah_data / $batas);

            $datatable ="SELECT * FROM data_history WHERE YEAR(tanggal) = YEAR(NOW()) AND MONTH(tanggal) IN (7) ORDER BY tanggal DESC LIMIT $halaman_awal, $batas";
            ...
            <?php $i++;?>
             <?php  }//endforeach;
              ...
              // $amal = $d["harga"]*(0.025);
              //////////////////////////////
                  $saldo1 = "SELECT SUM(harga) AS sum FROM data_history WHERE YEAR(tanggal) = YEAR(NOW()) AND MONTH(tanggal) IN (7) AND tipe_package = 'prawedding'";
                  ...
                  $saldo2 = "SELECT SUM(harga) AS sum FROM data_history WHERE YEAR(tanggal) = YEAR(NOW()) AND MONTH(tanggal) IN (7) AND package IN ('self photo','studio')";
                  ...
                  $saldo3 = "SELECT SUM(harga) AS sum FROM data_history WHERE YEAR(tanggal) = YEAR(NOW()) AND MONTH(tanggal) IN (7) AND tipe_package IN ('best deal','special package','diamond package','baby best deal','baby special package')";
