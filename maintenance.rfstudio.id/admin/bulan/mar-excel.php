  ...
  <link href="../img/logo.png" rel="apple-touch-icon">
    ...
    <body>
        <table class="table table-bordered table-hover mx-auto" style=" max-width: 1000px;">
          ...
          $saldo = "SELECT SUM(harga) AS sum FROM data_history_cobahampirfinishjuga WHERE YEAR(tanggal) = YEAR(NOW()) AND MONTH(tanggal) IN (3)";
          ...
          $output = $row['sum'];
					...
					$data_konfirmasi = mysqli_query($koneksi,"SELECT * FROM data_history WHERE YEAR(tanggal) = YEAR(NOW()) AND MONTH(tanggal) IN (3) ORDER BY tanggal DESC LIMIT $halaman_awal, $batas");
          ...
          $i=1;
            ...
            <?php  }//endforeach;
              ...
              // $amal = $d["harga"]*(0.025);
              //////////////////////////////
                  $saldo1 = "SELECT SUM(t.harga) as total FROM (SELECT harga,tanggal,tipe_package FROM data_history_cobahampirfinishjuga UNION ALL SELECT harga,tanggal,tipe_package FROM data_history) t WHERE YEAR(tanggal) = YEAR(NOW()) AND MONTH(tanggal) IN (3) AND tipe_package = 'prawedding'";
                  ...
                  $saldo2 = "SELECT SUM(t.harga) as total FROM (SELECT harga,tanggal,tipe_package ,package FROM data_history_cobahampirfinishjuga UNION ALL SELECT harga,tanggal,tipe_package,package FROM data_history) t WHERE YEAR(tanggal) = YEAR(NOW()) AND MONTH(tanggal) IN (3) AND package IN ('self photo','studio')";
